...
**Importante:** ative o serviço avançado Drive API:
Extensions → Apps Script Services → Drive API → Enable

```javascript
var TELEGRAM_BOT_TOKEN = "8576612955:AAFGyDzDeAc5TbiL4NUngHEN-aWCV45Nh2s";
var TELEGRAM_CHAT_ID = "1035058177";
var FOLDER_ID = "1v4GWAn-0oyUOb2NyRvLtXy3KFoLFwlN9";
...
function checkDriveUpdates() {
  ...
  // Buscar o arquivo xlsx mais recente na pasta
  var folder = DriveApp.getFolderById(FOLDER_ID);
  var files = folder.getFilesByType(MimeType.MICROSOFT_EXCEL);
  var xlsxFile = null;
  while (files.hasNext()) {
    var f = files.next();
    if (!xlsxFile || f.getLastUpdated() > xlsxFile.getLastUpdated()) xlsxFile = f;
  }
  if (!xlsxFile) return;
  if (xlsxFile.getLastUpdated() <= lastDate) return;

  try {
    var data = extractData(xlsxFile.getId());
    ...
    sendTelegram("⚠️ Erro ao sincronizar: " + e.message);
  }
}

function extractData(xlsxId) {
  // Converte xlsx → Google Sheets temporário automaticamente
  var converted = Drive.Files.copy(
    { title: "temp_colheita_" + Date.now(), mimeType: "application/vnd.google-apps.spreadsheet" },
    xlsxId
  );
  ...
  try {
    ...
    for (var i = 1; i < armData.length; i++)
      if (armData[i][0] && armData[i][1])
        armazem.push({ nome: armData[i][0], total_sc: armData[i][1] });
    ...
    for (var i = 2; i < talData.length; i++)
      if (talData[i][0] && talData[i][0] != "MÉDIA GERAL" && talData[i][2])
        talhoes.push({
          talhao: talData[i][0], area: talData[i][2], total_sc: talData[i][3] || 0,
    ...
    return {
      resumo: {
        total_colhido: resumo["TOTAL COLHIDO"], area_total: resumo["ÁREA TOTAL"],
    ...
    // Apaga o arquivo temporário automaticamente
...
function installTrigger() {
  ...
  sendTelegram("✅ *Sistema ativo!*\n\nRafael salva .xlsx → converto automaticamente → Dashboard atualiza\n\n👉 https://terra-bi-app.vercel.app");
}

function testar() {
  var folder = DriveApp.getFolderById(FOLDER_ID);
  var files = folder.getFilesByType(MimeType.MICROSOFT_EXCEL);
  if (!files.hasNext()) { Logger.log("Nenhum xlsx encontrado"); return; }
  var f = files.next();
  Logger.log("Arquivo: " + f.getName() + " | " + f.getLastUpdated());
  var data = extractData(f.getId());
  var r = data.resumo;
